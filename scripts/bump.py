...
cargo_paths = ["parser/Cargo.toml", "python/Cargo.toml"]
version_pattern = r'\nversion\s*=\s*"(\d+\.\d+\.\d+)"'
...
def check_in_and_tag(version):
    subprocess.run(["git", "add", pyproject_path] + cargo_paths + ["Cargo.lock"], check=True)
